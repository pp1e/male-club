...
import { IChild } from '../PersonalAccount/PersonalAccount';
...
const PrerecordingPage = (props: IProps): ReactElement => {
    ...
    async function getAvailableConsoles(enteredDateTime: Date) {
        const consoleList: IConsole[] = [];
        await getConsolesStatusList(enteredDateTime).then(result => {
            result.data.array.forEach((console: any) => {
    ...
    const onChange = (event: any) => {
        if (!!getDateValue() && !!getTimeValue()){            
            const curDateTime = new Date(+(new Date(`${getDateValue()!!} ${getTimeValue()!!}`)) + 4*3600000);
    ...
    const onSubmit = async (event: any) => {
        ...
        if (!getChildValue() || !getDateValue() || !getTimeValue() || !getPhoneValue ) {
            ...
            const curReservation: IReservation = {
                time_and_date: curDateTime,
                child_id: +getChildValue()!!,
                console_id: +getConsoleValue()!!,
                phone: getPhoneValue()!!
            };
            ...
            <div className="container mt-5 py-5 d-flex flex-row justify-content-center">
                <div className="text-center login-page_container">
                    ...
                    <form method="POST" onSubmit={onSubmit}>
                        ...
                        <div className="mb-5">
                            <select className={`form-select py-3 text-start border-warning ${getConsoleValue() ? 'prerecording-page__dark-text' : 'prerecording-page__light-text'}`}
                                    ref={consoleRef}
                            >
