...
const AddChildCard = ({ listChanged, setListChanged, child, isEdit, setEditIndex }: { listChanged: boolean, setListChanged: React.Dispatch<React.SetStateAction<boolean>>, setEditIndex: React.Dispatch<React.SetStateAction<number>>, child?: IChild, isEdit?: boolean }): ReactElement => {
    ...
    const onSubmit = async (event:any) => {
        ...
        const validationPassed = isEdit ? !!nameRef.current?.value : !!nameRef.current?.value && !!dateRef.current?.value && checkAge(new Date(dateRef.current?.value!!)) && new RegExp(dateRegExp).test(dateRef.current?.value);
        if (validationPassed) {
            if (isEdit) {
                        ...
                        <div className="d-flex flex-column justify-content-center align-items-center pt-3 account-page__container">
                            <div className="card__input__width mb-2">
                                <input
                                    ref={nameRef}
                                    name="name"
                                    defaultValue={isEdit ? child?.name : undefined}
                                ...
                                !isEdit  ?
                                        ...
                                        :
                                            <div className="d-flex flex-column justify-content-around py-1">
                                                <span className="card__text-main">Возраст: {getAge(child!!.date)}</span>
                                            </div>
                            ...
                            <div className="card__input__width mb-2">
                                <textarea
                                    ref={featuresRef}
                                    name="features"
                                    defaultValue={isEdit ? child?.features : undefined}
